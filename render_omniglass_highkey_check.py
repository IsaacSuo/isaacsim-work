"""High-key studio diagnostic for the cached OmniGlass soft body."""
...
source = source.replace(
    ...
    'shader.CreateInput("glass_color", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(1.0, 1.0, 1.0))',
)
source = source.replace(
    'shader.CreateInput("frosting_roughness", Sdf.ValueTypeNames.Float).Set(0.035)',
    'shader.CreateInput("frosting_roughness", Sdf.ValueTypeNames.Float).Set(0.018)',
)
source = source.replace(
...
source = source.replace(
    ...
    'settings.set("/rtx/post/tonemap/exposure", -0.15)',
)
source = source.replace(
    '"omniglass_normals_check_f000060.png"',
    '"omniglass_highkey_check_f000060.png"',
)
if "backdrop_shader" not in source or "omniglass_highkey_check" not in source:
